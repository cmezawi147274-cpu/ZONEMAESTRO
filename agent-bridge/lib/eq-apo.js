...
/** Windows' equivalents of "someone else has this file open right now" —
 ...
 *
 * That fixed writes racing *each other* (see withLock below), but not this
 * process racing Equalizer APO itself: MoveFileEx still fails with EBUSY
 * (sometimes EPERM/EACCES) if APO has config.txt open without
 * FILE_SHARE_DELETE at the exact instant of the rename — a few
 * milliseconds' overlap with APO's own reload, not a real conflict. Retried
 * a handful of times with a short backoff rather than surfaced straight to
 * the operator as a failed SET_EQ: the write itself is atomic and
 * idempotent, so retrying is always safe, and APO's own read is brief
 * enough that it has essentially always cleared by the second or third
 * attempt — total worst case (~5+10+15+20 = 50ms across 5 attempts) stays
 * far under AGENT_COMMAND_ACK_TIMEOUT_MS.
 */
async function writeFileAtomic(filePath, data, attempts = 5) {
 ...
 */
async function rebuildRootConfig(dir, mapping) {
  const lines = [
  ...
  await writeFileAtomic(path.join(dir, "config.txt"), lines.join("\r\n"));
}
...
function applyZoneEqualizer(zone, settings) {
  ...
  return withLock(dir, async () => {
    ...
    await writeFileAtomic(path.join(dir, file), withDevice);

    await rebuildRootConfig(dir, readMapping(dir));
...
function pruneStaleZones(validZoneIds) {
  ...
  return withLock(dir, async () => {
    ...
    if (removed.length) await rebuildRootConfig(dir, readMapping(dir));
    return removed;
  });
}
